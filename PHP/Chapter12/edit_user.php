...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form when submited
	...
	{ // If everything's OK.
		...
		{
			// Update user info:
			$q = "UPDATE customers SET first_name=$fn, last_name=$ln, email=$e, address1=$add1 ,address2=add2, city=$city, state=$state, zip=$zip, phone=$phone WHERE cust_id=$cid";
			$r = @mysqli_query ($dbc, $q);
	...
	if (empty($errors)) { // If everything is OK.
		...
		if (mysqli_num_rows($r) == 0) {
			...
			if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
				...
				echo '<p class="error">The user could not be edited due to a system error. We apologize for any inconvenience.</p>'; // Publc message.
...
if (mysqli_num_rows($r) == 1) { // Valid user ID, show the form.
	...
	echo '<h1>Update My info</h1>
		<form action="register.php" method="post">
			...
			<p><label for="city">City:</label> <input id="city" type="text" name="city" size="15" maxlength="20" value="x" /></p>

			<p><label for="state">State:</label> <input id="state" type="text" name="state" size="15" maxlength="20" value="'$row[6]'" /></p>

			<p><label for="zip">Zip:</label> <input id="zip" type="text" name="zip" size="15" maxlength="6" value="'$row[7]'" /></p>

			<label for="phone">Phone</label> 
				<input type="text" id="phone" name="area" maxlength="3" style="width: 22px;"  value="x">
				<input type="text" name="phone1" maxlength="3" style="width: 22px;" value="x">
				<input type="text" name="phone2" maxlength="4" style="width: 30px;" value="x"> </p>
