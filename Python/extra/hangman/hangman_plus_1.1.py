...
pygame.display.set_caption('Political HANGMAN!!')
...
option = [0,'Structured Voting','going WILLY NILLY','Political Info','Leave America',0,'',0]# menu options
...
guessed = [0,'Guess a letter in my secret phrase!',0]# [0-(0 invalid guess, 1 valid guess),1-Screen Prompt,2-Keyboard delay
infoDisplayText = ["i'm BERNIE",'','you ready to feel the BERN!','',"i'm not HILLARY ",'']# [0-5 Text, 2 per row]
banner =[50,50,0,0,0,0,0,0]# Start point for Hangman+ banner text, mountains etc
show = [0,52,True,True]# 0-Actual FPS, 1-Max FPS, 2-Gradient Sky, 3-Background
...
# set up the dictionary - First line of text file is categorys, the words must match the category order.
...
def infoDisplay(who,time):
    ...
    while count < time:
        ...
        if p[2] >= h[2]:# This sets the draw order so whoever is zoomed most is in front
            ...
            hangman()
        if count >14:
           if count <(time-11):# Hangman & Player have diffent speech bubble arrow locations
                if who==h:
...
def background():
    if show[2] == False:
        ...
        sky = pygame.draw.rect(windowSurface, LBLUE,((0,0,800,300)))
...
def backchat():
    neg='fat pig!+dog!+slob!+old HAG!+you women!!+rapist!+you junkie!!'.split('+')
    chat = 'BUILD THAT WALL!+Deported!+I have lots of friends+'.split('+')
    if h[5]==1:
        abuse = chat
    else:
        abuse = neg
...
def getGuess(alreadyGuessed):
    while event.type == KEYDOWN:
        ...
        if len(guess) != 1:
                    ...
                    guessed[1]='Guess a letter in my secret phrase!'
                    return guess
    else:
        guessed[0]=0
        guessed[2]=0
...
def displayBoard(display,missedLetters, correctLetters, secretWord):
    ...
    title = titleFont.render('Political Hangman', True, BLUE,)
    titles = titleFont.render('Political Hangman', True, BLACK,)
    ...
    scoreboard = basicFont.render('TRUMP ' + str(score[1]) + ' - BERNIE ' + str(score[0]) , True, WHITE,)
    scoreboards = basicFont.render('TRUMP ' + str(score[1]) + ' - BERNIE ' + str(score[0]) , True, BLACK,)
...
def menu():
    ...
    while option[5]==0:# Option [5] is the selection output bit
        ...
        if score[0] + score[1] > 0:
            ...
            scoreboard1 = basicFont.render('TRUMP ' + str(score[4]) + ' - BERNIE ' + str(score[3]) , True, WHITE,)
            scoreboard1s = basicFont.render('TRUMP ' + str(score[4]) + ' - BERNIE ' + str(score[3]) , True, BLACK,)
        ...
        text6s = bannerFont.render("Political Hangman",True, BLACK,)
        text6 = bannerFont.render("Political Hangman",True, BLUE,)
...
def about():
    while option[7]>=0:
        ...
        background()
        drawBanner()
        text1s = guessFont.render('Political Hangman is a spin off of another version. It was written', True, BLACK,)
        text1 = guessFont.render('Political Hangman is a spin off of another version. It was written', True, WHITE,)
        text2s = guessFont.render('on 04/26/2016 during political times,', True, BLACK,)
        text2 = guessFont.render('on 04/26/2016 during political times,', True, WHITE,)
        text3s = guessFont.render('It is written in Python 3.2 and Pygame 1.9.', True, BLACK,)
        text3 = guessFont.render('It is written in Python 3.2 and Pygame 1.9.', True, WHITE,)
        text4s = guessFont.render('The Hangman Logic is recreated from ' , True, BLACK,)
        text4 = guessFont.render('The Hangman Logic is from Al Sweigarts brilliant book', True, WHITE,)
...
menu()
start()
infoDisplayText = ["i'm wonderful DONALD",'',"trust me i'm RICH!!!!",'',"make america great again",'']
infoDisplay(h,int(show[1]*2.5))
infoDisplayText = ['Well done!','','You one step closer to free college ','Next Subject - ',secretKey]
while True:
...
##                    noooSound.play()
                    ...
                    if roundNo>8:
                        ...
                        score[3]+=1
                        playerWinSound.play()
                        infoDisplayText = ['Congratulationd!','','your qualified for president','','and beat the Political Hangman','']
                        infoDisplay(p,int(show[1]*2.5))
                        infoDisplayText = ['How did you beat me?','',"i'm firing everyone",'','You\'ll never do it again!','']
                        infoDisplay(h,int(show[1]*2.5))
                        gameIsDone = True
                    else:
                        yaySound.play()
                        ...
                        if option[5]==1:
                            ...
                            secretWord,secretKey = getRandomWord(words)
                        infoDisplayText = ['Well done!','','Your one step closer to free college ',str(roundNo+1),'Next Subject - ',secretKey]
                        gameIsDone = False
                        guessed=[0,'Guess a letter in my secret phrase!',0]
                        s = 0
                ...
                if len(missedLetters) == 6:
                            ...
                            gameover(p)
                            mwahahaSound.play()
                            infoDisplayText = ['your deported','','no food stamps','','Why waste your state benefits! ','',str(roundNo+1),'','The word was ','',secretWord.title()]
                            infoDisplay(h,int(show[1]*2.5))
                            infoDisplayText = ["I'm too old for this sh*t",'','Hey BILL can i borrow MONICA','',"I'm wealthy..NOT rich!!",'']
                            infoDisplay(p,int(show[1]*2.5))
                            gameIsDone = True
                            score[1]+=1
            ...
            if gameIsDone:
                    ...
                    if option[5]==1:
                        ...
                        secretWord,secretKey = getRandomWord(words)
                    infoDisplayText = ['Welcome to Political Hangman','','Round ',str(roundNo+1),'Subject - ',secretKey]
                    gameIsDone = False
                    guessed=[0,'Guess a letter in my secret phrase!',0]
                    s = 0
                    h = [280,259,1,40,40,0,100,100,0,0,150,240,13]# [0-X,1-Y,2-Scale,3-left hand,4-right hand,5-backchat,6-noose,7-new noose,8-blink count,9-blink timer]
